...
本章为 CodeGeeX4-ALL-9B 专用代码基座的 LoRA 微调与 pass@k 评测构建统一语料；其中基座说明与评测指标的技术背景可分别参见模型发布页与代表性评测工作\cite{thudm2024codegeex4all9b,chen2021evaluating}。具体要求有三：题目覆盖主流算法范式、解答代码可被单元测试自动判定正确性、并具备支持思维链监督与少样本检索的结构化标注。综合规模、质量与可执行性考虑，本文选取公开的 KodCode 作为原始来源\cite{xu2025kodcode}——其条目天然附带题目、参考解答与针对性单测，契合「生成—执行—判定」闭环的构建需要。
