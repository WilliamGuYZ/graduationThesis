...
本章为 Qwen2.5-Coder-7B-Instruct 的 LoRA 微调与 pass@k 评测构建统一语料；其中基座模型与评测指标的技术背景可分别参见官方技术报告与代表性评测工作\cite{qwen2025coder,chen2021evaluating}。具体要求有三：题目覆盖主流算法范式、解答代码可被单元测试自动判定正确性、并具备支持思维链监督与少样本检索的结构化标注。综合规模、质量与可执行性考虑，本文选取公开的 KodCode 作为原始来源\cite{xu2025kodcode}——其条目天然附带题目、参考解答与针对性单测，契合「生成—执行—判定」闭环的构建需要。
