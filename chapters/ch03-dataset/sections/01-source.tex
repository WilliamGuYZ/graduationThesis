...
\paragraph{（1）原始字段}
原始 KodCode 以 Parquet 列式存储，每条记录含四个字段：\texttt{question}（自然语言题目）、\texttt{solution}（Python 参考解答）、\texttt{test}（pytest 风格单测脚本）与 \texttt{test\_info}（被测函数签名的结构化描述，含 \texttt{function\_declaration}、\texttt{function\_name}、\texttt{parameter\_list} 等）。脚本 \texttt{convet\_KodCode\_to\_jsonl.py} 按顺序扫描原始文件、仅保留上述字段并逐条调用单测执行器，直至凑齐 \texttt{HEAD\_N}=10000 条目标样本。
