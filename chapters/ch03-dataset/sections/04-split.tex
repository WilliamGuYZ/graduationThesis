...
\paragraph{（2）规模与名义上界}
最终产出训练集 \pth{KodCode_train.jsonl}（约 $33$\,MB）与评测集 \pth{KodCode_eval.jsonl}（约 $3.8$\,MB、共 $1000$ 条）。在评测集上以原始 \texttt{solution} 配其自带 \texttt{test} 运行 \pth{validate_code_with_test}，参考解答 pass@1 实测为 $0.996$，作为后续所有模型侧实验的名义上界——任何训练/推理配置若逼近该值即可视为饱和，与 $1.000$ 的微小差距来自执行非确定性（浮点精度、系统时钟等）而非数据质量。
