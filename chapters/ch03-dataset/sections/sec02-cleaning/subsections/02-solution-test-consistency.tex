...
格式层只能保证字段形态正确，无法保证 \texttt{solution} 与 \texttt{test} 语义匹配；KodCode 中确实存在解答与测试版本不同步、依赖未声明、或测试覆盖的函数名与 \texttt{test\_info} 不一致等问题。为此 \pth{validate_code_with_test.py} 实现了轻量执行器 \texttt{test\_solution\_code}，在数据清洗与 pass@k 评测两个场景复用同一套判定逻辑，避免口径漂移。
