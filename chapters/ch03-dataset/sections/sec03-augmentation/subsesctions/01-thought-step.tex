...
思维步骤（\texttt{thought\_step}）是 CoT 监督与 CoT 推理两条路径的共同信号。为使其在不同训练侧（\texttt{LoRA\_cot\_zs}、\texttt{LoRA\_cot\_fs}）与推理侧（\texttt{CoT-ZS}、\texttt{CoT-FS}）配置下具备可比性，本文由 \texttt{annotate\_thought\_steps.py} 调用教师模型 \texttt{deepseek-chat} 统一离线生成，具体协议如下。
