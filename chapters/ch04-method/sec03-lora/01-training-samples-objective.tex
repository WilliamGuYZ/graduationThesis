...
\begin{itemize}
    \item \textbf{完整序列}：\pth{apply_chat_template(messages, tokenize=True)} 得到完整 token 序列 $T$；
    \item \textbf{非 assistant 前缀}：\pth{apply_chat_template(messages[:-1], add_generation_prompt=True)} 得到截至 assistant 起始标记的前缀 $P$；
...
\]
评分候选，取得分最高的前 $\max(k, 4k)$ 条作为候选池，再用 \texttt{random.sample} 从中随机抽取 $k$ 条拼在 user 段之前，构成 \texttt{(题目, 推理, 代码)} 形式的示例序列。该构造与 4.4.2 节评测侧的检索函数共享同一评分与同一强标签集合，仅在随机性来源上存在差异（训练侧 \texttt{random.sample}，评测侧用 \pth{seed = 2026 + problem_idx * 1000 + sample_idx} 做可复现抽样），以此保证 train/test 上下文统计一致而评测仍可稳定复现。
