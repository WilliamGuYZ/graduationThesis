...
\begin{itemize}
    ...
    \item \textbf{可调 CLI（训练）}：\texttt{train/train.py} 与 \texttt{train/train\_cot.py} 支持 \texttt{--data-path}、\texttt{--max-length} 覆盖默认 JSONL 与截断上限（论文表~\ref{tab:ch4-lora-optim} 为仓库默认值；显存不足时在命令行下调 \texttt{max-length} 与/或减小 batch，见 \texttt{README.md}）；
