...
\begin{description}
  \item[\texttt{Base}]
    ...
    \end{itemize}
  \item[\texttt{LoRA\_code}] 一轮目标对话（无 few-shot）：System 与 Direct 推理逐字同构，User 末尾追加「Only output the Python code, no explanation.」，Assistant 为不带 Markdown 围栏的可执行 Python 正文（与 \texttt{solution} 一致），loss 仅落最后一轮 Assistant；字面模板见附录~\ref{appendix:prompts}~A.1。
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item \textbf{字面模板}：上述 Stage-1 / Stage-2 与 CoT-FS 示例骨架的逐字模板见附录~\ref{appendix:prompts}~A.3、A.4。
    \end{itemize}
