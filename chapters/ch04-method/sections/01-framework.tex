...
第三章已得到可执行单测判定、带思维步骤与算法标签的统一语料。本章据此设计模型与方法层面的全部对照：以 CodeGeeX4-ALL-9B（\texttt{THUDM/codegeex4-all-9b}）为共同基座；工程目录约定为 \texttt{models/Qwen2.5-Coder-7B-Instruct/}，围绕「训练侧是否注入 LoRA 参数」与「推理侧是否显式走 CoT 两阶段」两条独立因子，建立覆盖主效应与交互效应的析因矩阵，为第五章 pass@k 层面的归因提供完整对照基础。
