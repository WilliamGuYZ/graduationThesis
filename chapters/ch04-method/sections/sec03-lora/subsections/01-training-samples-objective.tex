...
\begin{itemize}
    ...
    \item \textbf{长度约束}：\texttt{train/train.py}（\texttt{LoRA\_code}）固定 \texttt{MAX\_LENGTH=1024}；\texttt{train/train\_cot.py}（\texttt{LoRA\_cot\_*}）固定 \texttt{MAX\_LENGTH=2048}，以兼顾 CoT 监督目标段的完整保留与 32GB 单卡显存上限，避免 few-shot + CoT 在 4096 长度下出现反向传播阶段的 OOM；
