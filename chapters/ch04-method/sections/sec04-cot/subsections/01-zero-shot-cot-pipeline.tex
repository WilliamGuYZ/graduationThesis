...
\begin{itemize}
    ...
    \item \textbf{输出清洗}：若模型提前滑出任意三反引号代码块（\verb|```python|、\verb|```bash| 等任意语言标记或无标记围栏），立刻截断，只保留前置自然语言段，避免「推理阶段偷跑代码」污染第二阶段的上下文。
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{代码抽取}：\texttt{eval\_lora\_cot\_passk.py} 中取\textbf{最后一个} \verb|```python| 块（若无则取最后一个无语言围栏块，再退回全文），以便在模型输出多段围栏时仍以最终代码块为准；此逻辑与 4.2.2 节 Direct 路径「取首段围栏」不同，二者脚本实现已分叉，勿混读。
\end{itemize}
