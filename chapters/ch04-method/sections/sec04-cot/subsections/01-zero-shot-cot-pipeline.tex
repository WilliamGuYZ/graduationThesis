...
第二阶段接收 \{题目, 截断后的推理\} 作为 chat 上下文，要求输出可直接执行的代码：System 为「\texttt{expert Python programmer. Based on the reasoning, write the code.}」；Assistant 期望格式固定「推理文字 + 空行 + \verb|```python ... ```|」，与训练阶段 assistant 回复格式严格一致；采样参数 \texttt{MAX\_TOKENS\_CODE=1024}、其余与 Stage-1 相同。代码抽取：\texttt{eval\_lora\_cot\_passk.py} 中取\textbf{最后一个} \verb|```python| 块（若无则取最后一个无语言围栏块，再退回全文），以便在模型输出多段围栏时仍以最终代码块为准；此逻辑与 4.2.2 节 Direct 路径「取首段围栏」不同，两份 \texttt{extract\_code} 实现的 \texttt{[0]} / \texttt{[-1]} 差异详见附录~\ref{appendix:codes}~C.2。逐字 Stage-2 综合 User 模板见附录~\ref{appendix:prompts}~A.2。
