...
\subsubsection{Stage-1：推理生成}
第一阶段只负责产出自然语言推理，不允许直接给出代码：System 为「\texttt{expert algorithm teacher. Generate step-by-step reasoning.}」；User 采用与 \texttt{LoRA\_cot} 训练相同的 CoT 版题目指令（题面 + 末尾 \texttt{Think step by step, then output the Python code.}），\textbf{不是} 4.2.1 节 Direct 模板中的 \texttt{Only output the Python code, no explanation.}，以避免「要求只输出代码」与「本阶段只生成推理」相矛盾；采样温度与基线 Direct 一致。输出清洗：若模型提前滑出任意三反引号代码块（\verb|```python|、\verb|```bash| 等任意语言标记或无标记围栏），立刻截断只保留前置自然语言段，避免「推理阶段偷跑代码」污染第二阶段上下文。逐字模板见附录~\ref{appendix:prompts}~A.2.2。

\subsubsection{推理段的长度预算}
第一阶段输出不能无限拼进第二阶段，否则会压缩 Stage-2 的生成预算、并在 CoT-FS 模式下挤出示例内容。为此引入字符级与 token 级两级截断对推理段做硬上限控制，并相应放宽 vLLM 上下文窗口，为两阶段上下文与 few-shot 示例留出空间。

\subsubsection{Stage-2：代码生成}
第二阶段接收 \{题目, 截断后的推理\} 作为 chat 上下文，要求输出可直接执行的代码：System 为「\texttt{expert Python programmer. Based on the reasoning, write the code.}」；Assistant 期望格式固定「推理文字 + 空行 + \verb|```python ... ```|」，与训练阶段 assistant 回复格式严格一致。代码抽取规则与 Direct 路径略有不同：取生成文本中\textbf{最后一个}围栏代码块，以便在模型输出多段围栏时仍以最终代码块为准。逐字 Stage-2 综合 User 模板见附录~\ref{appendix:prompts}~A.2.2。

\subsubsection{端到端评测入口}
