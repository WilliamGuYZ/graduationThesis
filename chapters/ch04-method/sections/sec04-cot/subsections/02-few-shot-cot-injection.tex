...
两侧共用同一评分函数与同一强标签集合，差异仅在随机采样：训练侧使用全局 RNG，每个 epoch 的示例在统计上来自同一分布；评测侧将 seed 派生为题号与采样编号的确定性函数，同一 \texttt{(题目, 采样编号)} 的示例选择完全可复现，任意第三方可按此逐条复核。该设计保证训练与评测在「评分 + 候选池构造」上统计一致，同时让评测结果可被外部复现。

\subsubsection{示例拼接与预算分配}
$k$ 条示例在 Stage-1 与 Stage-2 中均以「\textbf{User（CoT 版题目指令）} $\rightarrow$ \textbf{Assistant（推理 + fenced Python）}」的多轮形态置于目标轮次之前（与 4.1 节（1b）一致）。长度预算上，示例推理保持原文不再截断（其长度已在数据生成阶段约束），仅对目标题的 Stage-1 输出做二级截断；vLLM 上下文窗口相应放宽，覆盖「全部示例 + 目标题 + 两阶段生成」。

\subsubsection{训练/推理 $k$ 匹配的强约束}
\begin{itemize}
    \item \textbf{规则来源}：4.1 节合法性约束；训练侧为 \texttt{LoRA\_cot\_fs($k$)} 时，推理侧 few-shot 条数必须同为 $k$；
    \item \textbf{脚本行为}：不匹配时评测脚本显式退出，从工程层面阻止误用；
