...
\begin{table}[!htb]
  ...
  \begin{tabular}{lllccc}
    ...
    \midrule
    H1 & Base               & Direct & \textit{TODO} & \textit{TODO} & \textit{TODO} \\
    H4 & LoRA\_code         & Direct & \textit{TODO} & \textit{TODO} & \textit{TODO} \\
    H7 & LoRA\_cot\_zs      & Direct & \textit{TODO} & \textit{TODO} & \textit{TODO} \\
    H9 & LoRA\_cot\_fs($k$) & Direct & \textit{TODO} & \textit{TODO} & \textit{TODO} \\
    \bottomrule
  \end{tabular}
\end{table}

% TODO: 跑完 H1/H4/H7/H9 后，将上表 TODO 占位替换为实际 pass@k 数值；
%       同步将本节文字中的「A/B/C」替换为具体百分点差值。
...
\end{itemize}
初步观察表明，训练侧从 Base 依次过渡到 \texttt{LoRA\_code}、\texttt{LoRA\_cot\_zs}、\texttt{LoRA\_cot\_fs($k$)} 时，pass@$k$ 在各档上均呈\textit{TODO：单调上升 / 先升后降 / 基本持平}（待实验数据回填）的走势——该走势是 5.3.1 节讨论 LoRA 有效性的起点。
