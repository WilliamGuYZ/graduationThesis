...
\subsubsection{数据集口径}
训练集与评测集分别来自第三章顺序划分产出的 \texttt{data/processed/KodCode\_train.jsonl}（约 9000 条）与 \texttt{KodCode\_eval.jsonl}（约 1000 条）。两份文件在全部 10 组实验中共享，保证不同方法面对同一批题目，具体字段与清洗策略见 3.1--3.4 节。评测集样本均满足「单测可判定且参考解答通过」硬约束，避免无效样本拉低指标下限。
...
\begin{itemize}
    \item \textbf{基座权重}、\textbf{tokenizer}、\textbf{chat template}、\textbf{bf16 精度}；
    \item \textbf{评测集}：同一份 \texttt{KodCode\_eval.jsonl}，不做抽样、不做重排；
    \item \textbf{采样参数}：\texttt{NUM\_SAMPLES=10}、\texttt{TEMPERATURE=0.3}、\texttt{top\_p=0.95}、\texttt{top\_k=50}；
    \item \textbf{执行环境}：同一子进程执行器、\texttt{TIMEOUT=5} 秒、同一 Python 解释器路径；
    \item \textbf{few-shot 检索}：强标签集合、评分函数、评测侧 seed 派生方式。
\end{itemize}
