...
实测：$H7=57.20\%$ vs $H8=63.90\%$，$H7-H8=-6.7$\,pp；$H9=58.50\%$ vs $H10=67.50\%$，$H9-H10=-9.0$\,pp。两组不等式方向均与 $H_i \gtrsim H_{i+1}$ 假设相反——\textbf{显式 CoT 两阶段推理始终优于训练侧内化 + Direct 推理}，差距在含 few-shot 的 $k$ 匹配档上进一步放大（−6.7\,pp $\to$ −9.0\,pp）。结论：\textbf{「CoT 进参数」与「CoT 进推理」并非互为替代，而是互补关系}——这一结论在 6.3.4 节实验扩展方向中将作为「混合推理策略」（在线根据预算与题型在 Direct / CoT 两条路径之间切换）的实证起点。
